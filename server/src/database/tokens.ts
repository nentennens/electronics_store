...
export async function getTokenByUser(userId: number): Promise<Token> {
	const [token]: any[] = await pool.query('SELECT * FROM tokens WHERE user_id = ?', [
		userId
	])
...
export async function createToken(userId: number, refreshToken: string): Promise<Token> {
	await pool.query(`INSERT INTO tokens (user_id, refresh_token) VALUES (?, ?)`, [
		userId,
		refreshToken
	])
...
export async function updateToken(userId: number, refreshToken: string): Promise<Token> {
	await pool.query('UPDATE tokens SET refresh_token = ? WHERE user_id = ?', [
		refreshToken,
		userId
	])
