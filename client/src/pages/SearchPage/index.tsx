...
export default function SearchPage(): React.ReactElement {
  ...
  if (!data.length && status === Status.FULFILLED) {
    return <h1 className={styles.statusHeader}>Nothing was found for the query "{query}" :(</h1>;
  }

  if (status === Status.REJECTED) return <h1 className={styles.statusHeader}>Failed to get items :(</h1>;
